...
int main() {
  ...
  while (flag == 1) {
    ...
    case (1): {
      ...
      clean();
      CARS.push_back(vehicle(vehicle_name, Nwheels));
      cout<<"ATTTENTIO"<<CARS[0]<<endl;
      //create_vehicle(adres, qty, car);
      rez = 0;
      break;
    }
    case (2): {
    ...
    case (4): {
      ...
      if (trackLen == 0) {
        ...
        for (int i = 0; i < qty; i++) {
          CARS[i].calculateRaceTime(trackLen);
          CARS[i].setMileage(trackLen);
          for (int j = 0; j < CARS[i].getNwheels(); j++) {
            CARS[i].ptr_wheel[j].def_wheel(trackLen,CARS[i].get_speed());
          }
            CARS[i].number_of_damaged_wheels();
            CARS[i].calculateRefuel(trackLen);
            CARS[i].calculate_cur_fuel(CARS[i].calculateIntake(), trackLen,CARS[i].get_NRefuel());
